...
ESP_STATIC_ASSERT(CONFIG_INTEGRATION_TIME % 20 == 0,
                  "INTEGRATION_TIME must be a multiple of 20");
...
static uint8_t calc_ssr_lvl(const ssr_control_msg_t *msg,
                            uint8_t last_ssr_lvl)
{
...
static void create_sigma_delta(uint8_t pattern[SSR_STEPS],
                               uint8_t requested_lvl, uint8_t n_on_up,
...
void ssr_coordinator(void *pv)
{
  ...
  const int64_t watchdog_timeout_us =
      (int64_t)CONFIG_SSR_WATCHDOG_S * 1000000LL;
...
esp_err_t ssr_init()
{
  ...
  ESP_RETURN_ON_ERROR(gpio_set_level(CONFIG_SSR_GPIO, 0), TAG,
                      "gpio_set_level()");
